                    ...
                    <table class="table table-striped">
                        <thead class="table-dark">
                            <tr>
                                ...
                                <th>Phone</th>
                                <th>
                                    <router-link to="/create" class="btn btn-secondary px-3">Create Student</router-link>
                                </th>
                            </tr>
                        </thead>
                        <tbody>
...
export default {
  ...
  created() {
      let apiURL_one = 'http://localhost:4000/api/getJobs';
      let apiURL_two = 'http://localhost:4000/api/getStudents';
      axios.get(apiURL_one).then(res => {
          this.JobPosts = res.data
      }).catch(error => {
          console.log(error)
      })

      axios.get(apiURL_two).then(res => {
